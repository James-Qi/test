...
The script downloads full chart payloads only to extract small corporate-action
records; it publishes OHLCV only for 2026-07-01..2026-08-19.
"""
...
SYMBOLS=U0+U1+U2
ALL_START=int(datetime(1990,1,1,tzinfo=timezone.utc).timestamp())
RECENT_START=int(datetime(2026,7,1,tzinfo=timezone.utc).timestamp())
...
def actions(symbol,chart):
 ...
 for key,typ in (("dividends","cash_dividend"),("splits","split"),("capitalGains","capital_gain")):
  for _,x in sorted(((chart.get("events") or {}).get(key) or {}).items(),key=lambda kv:int(kv[0])):
   t=x.get("date")
   out.append({"symbol":symbol,"action_type":typ,"ex_date":datetime.fromtimestamp(t,tz=timezone.utc).date().isoformat() if t else None,"cash_amount":x.get("amount"),"split_numerator":x.get("numerator"),"split_denominator":x.get("denominator"),"split_ratio":x.get("splitRatio"),"source":"yahoo_chart_v8"})
 return out
...
def main():
 out=Path("tmp/us_etf_r0_output"); out.mkdir(parents=True,exist_ok=True); recent=[]; action_rows=[]; failures={}
 for s in SYMBOLS:
  try:
   chart=fetch_chart(s,ALL_START,END); br=bars(s,chart); ar=actions(s,chart); recent+=br; action_rows+=ar
   print(s,"recent",len(br),"actions",len(ar),flush=True)
  except Exception as exc: failures[s]=f"{type(exc).__name__}: {exc}"
 write(out/"u2_recent_yahoo_20260701_20260819.csv",recent,["symbol","trade_date","open","high","low","close","adj_close","volume","source"])
 write(out/"u2_actions_yahoo_1990_20260819.csv",action_rows,["symbol","action_type","ex_date","cash_amount","split_numerator","split_denominator","split_ratio","source"])
 meta={"symbols":SYMBOLS,"recent_start":"2026-07-01","recent_end_inclusive":"2026-08-19","recent_rows":len(recent),"action_rows":len(action_rows),"failures":failures,"generated_at_utc":datetime.now(timezone.utc).isoformat()}; (out/"quality.json").write_text(json.dumps(meta,indent=2),encoding="utf-8"); print(json.dumps(meta,indent=2),flush=True)
